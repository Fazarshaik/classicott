...
const TimeMachine = () => {
  ...
  const handleDecadeClick = (decade) => {
    ...
    if (decade.period) {
      navigate(`/genre/${encodeURIComponent(decade.period)}`);
    }
  };
  ...
  return (
    <>
      <Home />
      <section className="py-16 bg-gradient-to-r from-amber-900/10 via-black to-amber-900/10  ">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="relative">
            <div className="flex items-center justify-center">
              <button
                onClick={scrollLeft}
                className="p-2 rounded-full bg-amber-600/20 hover:bg-amber-600/30 transition-colors mr-4"
              >
              ...
              <div
                ref={scrollRef}
                className="flex space-x-4 overflow-x-auto max-w-4xl no-scrollbar scroll-smooth"
              >
                {decades.map((decade) => (
                  <button
                    key={decade.value}
                    ...
                    {['top-2 left-2', 'top-2 right-2', 'bottom-2 left-2', 'bottom-2 right-2'].map((pos) => (
                      <div
                        ...
                        className={`absolute ${pos} w-3 h-3 ${
                          ...
                          selectedDecade === decade.value ? 'border-amber-200' : 'border-amber-600'
                        } opacity-50`}
                      />
                    ))}
                  </button>
              ...
              <button
                onClick={scrollRight}
                className="p-2 rounded-full bg-amber-600/20 hover:bg-amber-600/30 transition-colors ml-4"
              >
